...
public record OrderEvent() {
    ...
    public record Cancelled(
        ...
        Long userId,
        Long couponId,
        List<Cancelled.OrderItem> items,
        Long finalPrice
    ) {
        public static OrderEvent.Cancelled from(OrderModel order, Long couponId) {
            return new OrderEvent.Cancelled(
                order.getId(),
                order.getRefUserId(),
                couponId,
                order.getOrderItemModels().stream()
                    .map(item -> new Cancelled.OrderItem(
                            item.getId(),
                            item.getOrder().getId(),
                            item.getQuantity().getQuantity(),
                            item.getPurchasePrice().getAmount(),
